...
You can find a demo in the `examples/demo` folder, just run `bundle && rails s` and head to `http://127.0.0.1:3000`
...
3. Update the `project_id` and `account_id` values in the `PhraseApp::InContextEditor` configuration to match your trial account credentials
