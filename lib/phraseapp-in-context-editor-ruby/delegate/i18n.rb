...
private
  ...
  def covered_by_initial_caching?(key_name)
    key_name.start_with?(*InContextEditor.cache_key_segments_initial)
  end
  ...
  def translation_or_subkeys
    ...
    if keys.size == 1
      ...
      InContextEditor::HashFlattener.expand_flat_hash(translation_hash, @key)
    end
  end
  ...
  def build_cache
    cache = InContextEditor::Cache.new
  end
  ...
  def prefetched_key_names
    prefetched = Set.new
    InContextEditor.cache_key_segments_initial.each do |prefix|
  ...
  def key_names_from_nested(segment, data)
    key_names = Set.new
    InContextEditor::HashFlattener.flatten(data, nil) do |key, value|
