...
protected
  def to_be_translated_without_phrase?(args)
    InContextEditor.disabled? or has_been_given_blacklisted_key?(args) or has_been_given_ignored_key?(args) or has_been_forced_to_resolve_with_phrase?(args)
  end
  ...
  def key_is_ignored?(key)
    InContextEditor.ignored_keys.each do |ignored_key|
  ...
  def phrase_delegate_for(args)
    ...
    InContextEditor::Delegate::I18n.new(key, options, args)
  end
